      ...
      <section>
        <div className="flex items-center justify-between mb-4 px-1">
          <TextLTR className="flex items-center gap-2">
            <div className="text-text-muted dark:text-slate-400">{icon}</div>
            <h2 className="text-[11px] font-bold text-text-muted dark:text-slate-400 uppercase tracking-wider">{title}</h2>
          </TextLTR>
          <div className="text-[10px] font-bold text-primary px-2 py-0.5 bg-primary-light dark:bg-emerald-950/30 dark:text-emerald-400 rounded-full">{filteredVerbs.length} Kata</div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2">
          {filteredVerbs.length > 0 ? (
            ...
            <div className="py-12 text-center space-y-3 bg-stone-50/50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-border dark:border-slate-800">
               <div className="text-4xl opacity-20">{searchQuery ? '🔍' : '⭐'}</div>
               <div className="space-y-1">
                 <p className="text-[11px] text-text-dark dark:text-dark-text font-bold uppercase tracking-widest">
                    {searchQuery ? 'Kata Tidak Ditemukan' : 'Koleksi Masih Kosong'}
                 </p>
                 <p className="text-[10px] text-text-muted dark:text-slate-500">
                    {searchQuery ? 'Coba kata kunci lain atau periksa ejaan Anda' : 'Klik ikon bintang pada kata yang ingin Anda simpan'}
                 </p>
               </div>
               {searchQuery && (
